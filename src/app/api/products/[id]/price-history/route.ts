...
export async function POST(request: NextRequest, props: Props) {
  try {
    ...
    const product = await db.queryOne<any>('SELECT id, current_selling_price FROM products WHERE id = ?', [id]);
    if (!product) {
      return NextResponse.json(
    ...
    await db.transaction(async (tx) => {
      // 1. Insert new price history record
      await tx.execute(`
        INSERT INTO price_history (product_id, price, effective_from, note, created_by)
        VALUES (?, ?, ?, ?, ?)
      `, [id, newPrice, effective_from, note ? note.trim() : 'Điều chỉnh giá bán mới', user.id]);
